...
let handler = async (m, { conn, usedPrefix }) => {
  ...
  try {
    ...
    if (!levelling.canLevelUp(user.level, user.exp, global.multiplier)) {
      ...
      rank.build()
        .then(async data => {
          await conn.sendButtonImg(m.chat, data, `Level *${user.level} (${user.exp - min}/${xp})*\nKurang *${max - user.exp}* lagi!`.trim(), '©RadBotZ', 'Auto Level Up', `${usedPrefix}on autolevelup`, m, { thumbnail: data, height: 282, width: 934 })
        })
    }
    ...
    if (before !== user.level) {
      ...
      rank.build()
        .then(async data => {
          await conn.sendButtonImg(m.chat, data, `_*Level Up!*_\n_${before}_ -> _${user.level}_`.trim(), '©RadBotZ', 'Auto Level Up', `${usedPrefix}on autolevelup`, m, { thumbnail: data, height: 282, width: 934 })
        })
    }
  }
}
