...
let handler = async (m, { conn, command }) => {
  if (/^tod$/i.test(command)) {
    conn.send3Button(m.chat, 'Truth or Dare', '©RadBotZ', 'Truth', ',truth', 'Dare', ',dare', 'RANDOM', `${pickRandom([',dare', ',truth'])}`, m)
  }
  if (/^truth$/i.test(command)) {
    ...
    conn.send2Button(m.chat, json.message, '©RadBotZ', 'Truth', ',truth', 'Dare', ',dare', m)
